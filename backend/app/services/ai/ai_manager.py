"""
AI Manager for handling multiple AI providers
"""
...
class AIManager:
    ...
    def _initialize_providers(self):
        ...
        # Get API keys from database settings first, fallback to environment
        ...
        # Set default provider from settings if available
        ...
        if openai_key:
            try:
                ...
                logger.info("OpenAI provider initialized", source="database" if self._get_api_key("openai") else "environment")
        ...
        if anthropic_key:
            try:
                ...
                logger.info("Anthropic provider would be initialized here", source="database" if self._get_api_key("anthropic") else "environment")
            except Exception as e:
        ...
        if google_key:
            try:
                ...
                logger.info("Google provider would be initialized here", source="database" if self._get_api_key("google") else "environment")
            except Exception as e:
