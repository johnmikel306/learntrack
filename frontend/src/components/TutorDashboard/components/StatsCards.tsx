    ...
    return (
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 lg:gap-6">
        {Array.from({ length: 4 }).map((_, index) => (
          <Card key={index} className="border-0 shadow-sm bg-slate-50 dark:bg-slate-800">
            <CardContent className="p-4 sm:p-6">
              <div className="animate-pulse">
                <div className="h-10 w-10 bg-slate-300 dark:bg-slate-700 rounded-lg mb-4"></div>
        ...
        return (
          <Card key={index} className={`border-0 shadow-sm ${stat.bgColor}`}>
            <CardContent className="p-4 sm:p-6">
              ...
              <div>
                <p className="text-xs sm:text-sm font-medium text-slate-600 dark:text-slate-400 mb-1">{stat.title}</p>
